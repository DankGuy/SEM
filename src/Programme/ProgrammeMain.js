    ...
    useEffect(() => {
        ...
        if (filterType) {
            filteredProgrammes = filteredProgrammes.filter(item =>
                item.studyLevel.toLowerCase() === filterType.toLowerCase()
            );
        }
                    ...
                    {compareMode ? (
                        <div>
                            <input
                                ...
                                disabled={selectedForComparison.length >= 2 && !selectedForComparison.includes(programme.id)}
                            />
                            <span>{programme.programme_name}</span>
                        </div>
                    ) : (
                        <Card sx={{ maxWidth: 345 , minHeight:355}} style={{marginBottom: 50}}>
                            <CardActionArea>
                                <CardMedia
                                    component="img"
                                    height="140"
...
const SearchProgrammes = () => {
    ...
    return (
        <div className="container">
            <div>
                <Form form={form} onValuesChange={handleSearchChange}  className="searchBox">
                ...
                <div style={{ display: 'flex' }} className="category">
                    <Button onClick={() => handleFilterClick('bachelor')}>Bachelor</Button>
